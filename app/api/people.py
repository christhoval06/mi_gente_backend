...
@people_router.route('/people', methods=['GET'])
def get_people():
...
@people_router.route('/people/<int:id>', methods=['GET'])
def get_person(id):
...
@people_router.route('/people', methods=['POST'])
def set_person():
...
@people_router.route('/people/<int:id>', methods=['PUT'])
def put_person(id):
    person = Person.query.get(id)
    if person is None:
        return jsonify(error="person not found", errno=1), 403
...
@people_router.route('/people/<int:id>', methods=['PATCH'])
def patch_person(id):
    person = Person.query.get(id)
    if person is None:
        return jsonify(error="person not found", errno=1), 403
...
@people_router.route('/people/voting_place', methods=['GET'])
...
@people_router.route('/people/voting_place/stats', methods=['GET'])
...
@people_router.route('/people/voting_place/votes', methods=['GET'])
